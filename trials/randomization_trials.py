...
for seed in range(5):
    filepath = os.path.join(os.getcwd(), 'results', 'mnist_5layer_mlff_randomization', 'swni_s%d'%(seed))
    os.mkdir(filepath)
    log_file = open(os.path.join(filepath, 'log.txt'), 'a')
    sys.stdout = log_file
    trial['topology']['bypass p'] = float(0)
    trial['configuration']['seed'] = seed
...
trial = mnist_5layer_mlff

for seed in range(5):
    filepath = os.path.join(os.getcwd(), 'results', 'mnist_5layer_mlff_randomization', 'mlff_s%d'%(seed))
    os.mkdir(filepath)
    log_file = open(os.path.join(filepath, 'log.txt'), 'a')
    sys.stdout = log_file
    trial['configuration']['seed'] = seed
